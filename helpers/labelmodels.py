...
class BaseLabelModel(ABC):
    ...
    def score(self, label_matrix_valid: np.ndarray, true_labels: np.ndarray, metrics: Optional[List[str]] = None, **kwargs) -> Union[List[float], List[np.ndarray]]:
        ...
        Args:
            ...
            metrics (Optional[List[str]], optional): List of metrics to evaluate. Defaults to None.
...
class FlyingSquid(BaseLabelModel):
    ...
    def fit(self, label_matrix: np.ndarray, class_balance: Optional[np.ndarray] = None, ABSTAIN: int = -1, dependency_graph: Optional[List] = [], verbose: Optional[bool] = False, **kwargs) -> None:
        """Fit the FlyingSquid model to the label matrix.

        Args:
            label_matrix (np.ndarray): Label matrix.
            class_balance (Optional[np.ndarray], optional): Class balance. Defaults to None.
            ABSTAIN (int, optional): Value representing abstain. Defaults to -1.
            dependency_graph (Optional[List], optional): Dependency graph. Defaults to [].
            verbose (Optional[bool], optional): Verbose output. Defaults to False.
        """
        if class_balance is None:  # All classes have equal weight
            class_balance = np.ones(self.cardinality) / self.cardinality 
