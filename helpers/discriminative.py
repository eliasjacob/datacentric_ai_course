# TODO check if we really need this complexity for a class
# FIXME
...
class DiscriminativeModule(L.LightningModule):
    ...
    Args:
        ...
        arch (int, optional): The architecture of the network to use. Defaults to 1.
        dropout (float, optional): The dropout rate to use in the network. Defaults to 0.5.
        class_weights (Optional[np.ndarray], optional): The class weights to use for training. Defaults to None.
    """
    def __init__(self, input_size: int, output_size: int, learning_rate: float, arch: int = 1, dropout: float = 0.5, class_weights: Optional[np.ndarray] = None):
        super().__init__()
    ...
    def _convert_class_weights_to_loss_weights(self, class_weights: Optional[np.ndarray]) -> Optional[torch.Tensor]:
        """
        Convert class weights to loss weights.

        Args:
            class_weights (Optional[np.ndarray]): The class weights to convert.

        Returns:
            Optional[torch.Tensor]: The converted class weights as a PyTorch tensor, or None if class_weights is None.
        """
...
class SimpleDataModule(L.LightningDataModule):
    ...
    Args:
        train_data (Optional[List[Tuple[np.ndarray, np.ndarray]]]): A list of tuples containing the input and target data for training.
        val_data (Optional[List[Tuple[np.ndarray, np.ndarray]]]): A list of tuples containing the input and target data for validation.
        test_data (Optional[List[Tuple[np.ndarray, np.ndarray]]]): A list of tuples containing the input and target data for testing.
    ...
    """
    def __init__(self, train_data: Optional[List[Tuple[np.ndarray, np.ndarray]]] = None,
    ...
    def setup(self, stage: Optional[str] = None):
        ...
        Args:
            stage (Optional[str]): The stage of training (fit/test/predict). Defaults to None.
        """
    ...
    def train_dataloader(self) -> Optional[DataLoader]:
        ...
        Returns:
            Optional[DataLoader]: The dataloader for training data, or None if no training data is available.
        """
    ...
    def val_dataloader(self) -> Optional[DataLoader]:
        ...
        Returns:
            Optional[DataLoader]: The dataloader for validation data, or None if no validation data is available.
        """
    ...
    def test_dataloader(self) -> Optional[DataLoader]:
        ...
        Returns:
            Optional[DataLoader]: The dataloader for test data, or None if no test data is available.
        """
...
class SimpleDiscriminativeClassifier(BaseEstimator, ClassifierMixin):
    ...
    def __init__(self, input_size: int, output_size: int, arch: int = 1, learning_rate: float = 3e-4,
                 dropout: float = 0.5, batch_size: Optional[int] = None, max_epochs: int = 20,
                 ...
                 seed: int = 314, class_weights: Optional[np.ndarray] = None):
    ...
    def fit(self, X_train: np.ndarray, y_train: np.ndarray, sample_weights: Optional[np.ndarray] = None) -> 'SimpleDiscriminativeClassifier':
        """
        ...
        Args:
            ...
            sample_weights (Optional[np.ndarray], optional): The sample weights to use for training. Defaults to None.
    ...
    def predict_proba(self, X: np.ndarray, batch_size: Optional[int] = None) -> np.ndarray:
        ...
        Args:
            ...
            batch_size (Optional[int], optional): The batch size to use for prediction. Defaults to None.
...
def train_and_get_preds(
    ...
    y_test_prob: Optional[np.ndarray] = None,
    arch: int = 2,
...
) -> Dict[str, Union[str, np.ndarray, float]]:
    """
    ...
    Args:
        ...
        X_test (Optional[np.ndarray], optional): The input data for testing. Defaults to None.
        y_test_prob (Optional[np.ndarray], optional): The target probabilities for testing. Defaults to None.
        ...
        class_weights (Optional[np.ndarray], optional): The class weights to use for training. Defaults to None.
        sample_weights (Optional[np.ndarray], optional): Sample weights for training. Defaults to None.
