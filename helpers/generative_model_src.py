...
class SrcGenerativeModel:
    ...
    def __init__(self, class_prior=False, lf_prior=False, lf_propensity=False,
        ...
        if parse(numbskull_version) < parse(numbskull_require):
            raise ValueError(
    ...
    def predict_proba(self, X, batch_size=None):
        ...
        """
        if self.cardinality > 2:
            return self.marginals(X, batch_size=batch_size)
        else:
            positive = self.marginals(X, batch_size=batch_size)
            negative = 1 - positive
    ...
    def train(self, L, deps=(), LF_acc_prior_weights=None,
              ...
              burn_in=5, cardinality=None, timer=None, candidate_ranges=None, threads=10):
        """
    ...
    def marginals(self, L, candidate_ranges=None, batch_size=None):
        ...
        # Categorical setting
            ...
            for i in range(m):
                ...
                # Get softmax
                marginals = marginals - np.max(marginals)
    ...
    def _compile(self, L, init_deps, init_class_prior, LF_acc_prior_weights, is_fixed, cardinalities):
        ...
        n_weights += nPrior
        n_weights += nUnFixed
        for optional_name in SrcGenerativeModel.optional_names:
            ...
            if mat.nnz > 0:
                ...
                for i in range(len(mat.data)):
                    f_off, ftv_off, w_off = self._compile_dep_factors(L, factor,
                                                                      f_off, ftv, ftv_off, w_off, mat.row[i],
                                                                      mat.col[i],
    ...
    def _process_learned_weights(self, L, fg, LF_acc_prior_weights, is_fixed):
        ...
        weights = SrcGenerativeModelWeights(n)
...
class SrcGenerativeModelWeights(object):
    ...
    def is_sign_sparsistent(self, other, threshold=0.1):
        ...
        for name in SrcGenerativeModel.optional_names:
            for i in range(self.n):
                if not self._weight_is_sign_sparsitent(
                        getattr(self, name)[i], getattr(other, name)[i], threshold):
                    return False

        for name in SrcGenerativeModel.dep_names:
            for i in range(self.n):
